...
fn test_builtin_types_parsing() {
    ...
    assert_eq!(builtin_int.name(), "i32");
    assert_eq!(builtin_int64.name(), "i64");
    assert_eq!(builtin_float.name(), "f32");
    assert_eq!(builtin_bool.name(), "bool");
    assert_eq!(builtin_char.name(), "char");
    assert_eq!(builtin_double.name(), "f64");
...
fn test_generic_types_parsing() {
    let mut types_parser =
        create_parser_with_source("Generic1<i32, string> module1::Generic2<i32>");
    ...
    assert_eq!(generic_type.generics()[0].to_string(), "i32");
    assert_eq!(generic_type.generics()[1].to_string(), "string");

    assert_eq!(generic_type_with_modules.generics().len(), 1);
    assert_eq!(generic_type_with_modules.generics()[0].to_string(), "i32");
...
fn test_closure_types_parsing() {
    let mut types_parser = create_parser_with_source("closure(i32, string) closure() => string");
    ...
    assert_eq!(closure_type1.generics()[0].to_string(), "i32");
...
fn test_function_types_parsing() {
    let mut types_parser = create_parser_with_source("(void)(i32, string) (i32)(opaque)");
    ...
    assert_eq!(function_type1.generics()[0].to_string(), "i32");
...
fn from_string_desugars_optional_over_array() {
    // `i32[]?` is an optional array: Option<i32[]>.
    let t = PekoType::from_string("i32[]?", "<inline>");
    assert_eq!(t.name(), "Option");
...
fn from_string_parses_reference_depth() {
    ...
    let tt = PekoType::from_string("&&i32", "<inline>");
...
fn from_string_roundtrips_through_display() {
    ...
    for input in [
        ...
        "const i32",
        "const Foo<i32>",
        "module::Foo<i32, string>",
        "closure(i32, string) => bool",
        "(i32)(string, bool)",
    ] {
...
fn depth_excludes_predicates_from_matching() {
    let mut t = PekoType::simple_type("i32");
    t.array_depth = 1;
...
fn from_string_parses_leading_const() {
    let t = PekoType::from_string("const i32", "<inline>");
    assert_eq!(t.name(), "i32");
    assert!(t.is_const());

    let plain = PekoType::from_string("i32", "<inline>");
...
fn optional_get_inner_type_unwraps_option_generic() {
    ...
    t.generics_mut().push(PekoType::simple_type("i32"));
...
fn optional_get_inner_type_returns_none_for_non_optional() {
    let t = PekoType::simple_type("i32");
    assert!(t.optional_get_inner_type().is_none());
}
...
fn is_string_type_matches_all_string_forms() {
    ...
    assert!(PekoType::from_string("&char", "").is_string_type());
    assert!(!PekoType::from_string("i32", "").is_string_type());
}
...
fn display_matches_to_string() {
    ...
    let t = PekoType::from_string("module::Foo<i32>[]", "");
    assert_eq!(format!("{t}"), t.to_string());
}

#[test]
fn v2_ffi_scalar_names_canonicalize_to_internal_names() {
    assert_eq!(PekoType::from_string("i16", "").name(), "i16");
    assert_eq!(PekoType::from_string("i32", "").name(), "i32");
    assert_eq!(PekoType::from_string("i64", "").name(), "i64");
    assert_eq!(PekoType::from_string("i128", "").name(), "i128");
    assert_eq!(PekoType::from_string("f32", "").name(), "f32");
    assert_eq!(PekoType::from_string("f64", "").name(), "f64");
}
